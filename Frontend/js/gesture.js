...
function calcOpenness(lm) {
  ...
  [4, 8, 12, 16, 20].forEach(
    (i) => (avg += Math.hypot(lm[i].x - cx, lm[i].y - cy)),
  );
  avg /= 5;
...
export async function startCam({ onDetected } = {}) {
  ...
  if (!window.isSecureContext) {
    toast(
      "摄像头需要在 localhost 或 https:// 环境下使用，当前页面不符合要求",
      "error",
      7000,
    );
    return false;
  ...
  try {
    ...
    const isUnsecure =
      err?.message?.includes("getUserMedia") && !window.isSecureContext;
    toast(
      isPermission
...
export function stopCam() {
  ...
  _onDetected?.(false, 0);
  toast("摄像头已关闭，粒子缩放已恢复默认大小", "info", 2500);
}
